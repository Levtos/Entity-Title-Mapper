...
async def async_setup_entry(
    ...
    """Set up ETM number entities for a watcher."""
    runtime: WatcherRuntime = hass.data[DOMAIN][entry.entry_id]
    async_add_entities([EtmCurrentTitleEnumNumber(runtime)])


class EtmCurrentTitleEnumNumber(NumberEntity):
    ...
    def __init__(self, runtime: WatcherRuntime) -> None:
        ...
        self.entity_id = f"number.etm_{slug}_current_title_enum"
    ...
    async def async_set_native_value(self, value: float) -> None:
        ...
        enum = int(value)
        if enum != value:
            raise HomeAssistantError("ETM enum must be a whole number")
        if enum < MIN_ENUM or enum > MAX_ENUM:
            raise HomeAssistantError(f"ETM enum must be between {MIN_ENUM} and {MAX_ENUM}")
