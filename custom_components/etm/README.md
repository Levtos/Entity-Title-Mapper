...
After installing ETM, a **Title Classifier** sidebar entry (icon: tag-multiple) appears in Home Assistant. It requires admin access.
...
| Entity | Description |
|--------|-------------|
| `sensor.etm_<name>_enum` | Current mapped enum (0–9). Use this in automations. |
| `sensor.etm_<name>_raw` | Current raw title string (diagnostic). |
| `sensor.etm_<name>_catalog` | Number of tracked titles; full catalog in attributes (diagnostic). |

## Services

| Service | Description |
|---------|-------------|
| `etm.set_enum` | Map a specific title to an enum value. |
| `etm.delete_entry` | Remove a title from storage. |
| `etm.import_entries` | Bulk-import a list of `{key, enum}` mappings. |
| `etm.clear_old` | Delete entries not seen for N days. |
...
|---------|---------------|-------------|
| `etm/list` | No | All watchers with full catalog (legacy panel command). |
| `etm/set_enum` | No | Map title → enum. |
| `etm/delete_entry` | No | Delete a title entry. |
| `etm/import_entries` | No | Bulk-import mappings. |
| `etm/get_sources` | **Yes** | List all watchers (name, entry_id, watcher_type). |
| `etm/list_entries` | **Yes** | Flat list of all entries, optionally filtered by `source`, `unclassified`, `search`. |
| `etm/update_entry` | **Yes** | Update enum for one entry (`entry_id`, `key`, `enum_value` 0–9). |

## Permissions

The panel itself is registered with `require_admin=True`, so only HA admin accounts can see or use it. The three new WebSocket commands (`etm/get_sources`, `etm/list_entries`, `etm/update_entry`) also enforce the admin check server-side via `@websocket_api.require_admin`.
